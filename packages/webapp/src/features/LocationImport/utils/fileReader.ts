    ...
    reader.onload = (event: ProgressEvent<FileReader>) => {
      try {
        if (!event.target || !event.target.result) {
          reject(new Error('Failed to read file: Event target or result is null.'));
          return;
        }
        ...
        if (sheetIdentifier === undefined || sheetIdentifier === null) {
          ...
          if (!workbook.SheetNames.includes(sheetIdentifier)) {
            reject(new Error(`Sheet with name "${sheetIdentifier}" not found.`));
            return;
          }
          sheetName = sheetIdentifier;
        } else if (typeof sheetIdentifier === 'number') {
          // If identifier is a number, use it as a 0-based index
          if (sheetIdentifier < 0 || sheetIdentifier >= workbook.SheetNames.length) {
            reject(new Error(`Sheet index ${sheetIdentifier} is out of bounds.`));
            return;
          }
          sheetName = workbook.SheetNames[sheetIdentifier];
        } else {
          reject(new Error('Invalid sheet identifier type. Must be a string or number.'));
          return;
        }

        if (!sheetName) {
          reject(new Error('No sheets found or specified sheet could not be determined.'));
          return;
        }
        ...
        if (e instanceof Error) {
          ...
          reject(new Error('An unknown error occurred while processing the Excel file.'));
        }
      }
    };
