...
export const SourceFileInstructions: React.FC<SourceFileInstructionsProps> = ({
  dataSource,
}) => {
  if (!dataSource) {
    return null;
  ...
  if (dataSource === LocationDataSource.PROPERTY_FINDER) {
    instructions = (
      <>
        <Text size="sm" fw={500} mb="xs">
          Expected Excel Format for Property Finder:
        </Text>
        <List
          spacing="xs"
          size="sm"
        ...
        >
          ...
          <List.Item>
            The first row should contain headers (it will be skipped).
          </List.Item>
          <List.Item>
    ...
    instructions = (
      <>
        <Text size="sm" fw={500} mb="xs">
          Expected Excel Format for Bayut:
        </Text>
        <List
          spacing="xs"
          size="sm"
        ...
        >
          ...
          <List.Item>
            ...
            <List withPadding listStyleType="disc" size="xs" spacing={3} mt={3}>
              <List.Item>
                Column A (Index 0): Emirate (e.g., Dubai) - used as 'City'
              </List.Item>
              <List.Item>
                Column B (Index 1): Location Name (e.g., Mirdif, The Legends,
                Carmen) - used to determine the most specific part of the
                hierarchy
              </List.Item>
              <List.Item>
                Column C (Index 2): Location Type (e.g., Neighbourhood,
                Building, Cluster) - used as 'Location Type'
              </List.Item>
              <List.Item>
                Column D (Index 3): Location Hierarchy (e.g., Dubai{">"}Mirdif,
                Dubai{">"}DAMAC Hills{">"}The Legends) - used as 'Location Path'
                and to derive Community/Subcommunity/Property
              </List.Item>
            </List>
          </List.Item>
        </List>
