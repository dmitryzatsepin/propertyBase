...
export const useLocationImport = () => {
  ...
  const setSelectedDataSource = (source: LocationDataSource | null) =>
    dispatch({ type: ActionType.SET_SELECTED_DATA_SOURCE, payload: source });
  const setUploadedFile = (file: File | null) =>
    dispatch({ type: ActionType.SET_UPLOADED_FILE, payload: file });
  const updateColumnMapping = (mapping: ColumnMapping) =>
    dispatch({ type: ActionType.UPDATE_COLUMN_MAPPING, payload: mapping });

  const readFileAndInitiateMapping = async () => {
    dispatch({ type: ActionType.PROCESS_FILE_START });
    try {
      const result = await fileProcessingService.readFileForMapping({
      ...
      const errorMessage =
        e instanceof Error ? e.message : "Error reading Excel file.";
  ...
  const applyMappingAndProcessData = async () => {
    dispatch({ type: ActionType.PROCESS_FILE_START });
    try {
      const processedData = await fileProcessingService.processDataWithMapping({ // <--- Используем fileProcessingService
      ...
      const errorMessage =
        e instanceof Error ? e.message : "Error processing data with mapping.";
      dispatch({ type: ActionType.PROCESS_FILE_ERROR, payload: errorMessage });
    }
  };
  ...
  const saveProcessedDataToDB = async (): Promise<void> => {
    if (!state.processedData || state.processedData.length === 0) {
      dispatch({
        type: ActionType.SAVE_DATA_ERROR,
        payload: "No processed data to save.",
      });
      return;
    }
    ...
    try {
      ...
      dispatch({
        ...
        payload: `Successfully saved ${result.count} locations.`,
      });
    } catch (e) {
      const errorMessage =
        e instanceof Error
          ? e.message
          : "An unknown error occurred while saving data.";
